...
| §5 | Bitbucket Cloud silently drops a bare `token` | True, and **worse in three ways**: config validation does not reject it; a bare `appPassword` is discarded identically, because `username` is what the Basic scheme needs and it does not matter which key the secret was written under; and `token: null` is the safe spelling that `token: ''` is not |
| §5 | "All three public hosts get an auto-injected default integration" | **Four.** Azure also self-defaults — a bare `dev.azure.com` entry is appended when none names that host. Credential-less, so it does not change the anonymous-read story, but `byUrl` is truthy for Azure DevOps URLs too |
| §9 S1 | `integrations.byUrl()` as the SSRF guard | Even weaker than the four self-defaults suggest: `GithubIntegrationConfig.apiBaseUrl` is **optional** and deduced only for github.com, so a configured GHE entry omitting it sent enterprise repo paths and `If-None-Match` to the public API — a 404 for private repos and a path disclosure. Filed as latent; it was live |
