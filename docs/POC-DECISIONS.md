...
| # | Decision | Rationale |
|---|----------|-----------|
| D1 | **Self-contained demo + documented private path.** Sample collections are committed into the repo; the backend fetches them through Backstage's `UrlReaderService` — **public** repos read anonymously (no token), **private** repos read via the requesting user's GitHub OAuth token. The private-GitHub-repo code path is wired and documented, but the demo does not *require* live private creds to run. | Keeps the POC runnable by anyone who clones it; still exercises the RISK #1 credential-isolation path (Q4). |
...
└───────────────────────────────┘        └──────────────────────────────────────────┘
        ...
        ▼                                               ▼
  /api/proxy/*  (Backstage proxy-backend, CORS pass-through)   GitHub (anonymous for public; user OAuth token for private; host App/PAT optional) / local sample-collections/
```
...
| Q4 | Private-repo fetch, no service creds to browser, egress only Backstage→GitHub (RISK #1) | 🟢 Built + reasoned | **Public** repos are read anonymously via `UrlReaderService` (`integrations.github` is tokenless by default — `GITHUB_TOKEN` is commented out in `.env`). **Private** repos are read with the **requesting user's GitHub OAuth token** (`repo` scope): the frontend obtains it and sends it in the `x-bruno-github-token` header; the backend uses it for one Octokit fetch and **never persists or logs it** (only the parsed collection JSON crosses back to the browser). A host GitHub App/PAT is **optional**; if configured it's resolved server-side via `DefaultGithubCredentialsProvider`. The **service/App credential never reaches the browser**; the user's private-repo token is client-obtained **by design**. The **only network egress is Backstage→GitHub**. |
...
- **RISK #1 (private-repo, server-side fetch, no *service* creds to browser).** Cleanly solved with Backstage's native `UrlReaderService`. Public repos read anonymously (`integrations.github` is tokenless by default — `GITHUB_TOKEN` commented out); private repos read via the requesting user's GitHub OAuth token, sent in the `x-bruno-github-token` header, used for one fetch and never persisted or logged. The **service/App credential** never leaves the backend; the user's own private-repo token is client-obtained **by design**. The only egress is Backstage→GitHub. This is standard Backstage territory — **no blocker**. (Live private run is a config toggle; the code path is built and exercised against local + URL sources.)
