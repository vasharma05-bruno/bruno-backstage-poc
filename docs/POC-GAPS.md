...
- **Current:** All 3 active `bruno.sources` are `type: local` (`app-config.yaml`). The remote `url` source is present but **commented out** (`app-config.yaml`, ~lines 107–113) and has never been exercised at runtime. The code path itself is complete and live, **not** a stub: `readUrlTree()` calls `UrlReaderService.readTree()` (`plugins/bruno-backend/src/service/collectionService.ts:305–324`), and `loadSource()` already dispatches on `source.type` (`collectionService.ts:147–159`). The `integrations.github` block (`app-config.yaml`, ~lines 50–59) is wired to `${GITHUB_TOKEN}`.
- **To close:**
  ...
  - *Private half (needs a real private repo + `GITHUB_TOKEN`):* point the `url` source at a private repo, set the env var, and verify — via the browser network log — that the token never crosses to the client and the only egress is Backstage→GitHub.
