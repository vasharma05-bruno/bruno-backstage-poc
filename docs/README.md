...
| **[MULTI-SCM-PLAN.md](./MULTI-SCM-PLAN.md)** | A *separate* plan (DRAFT): supporting **GitLab / Bitbucket / other SCM providers** alongside GitHub. Verified Backstage capability matrix, the ten blockers (B1–B10) with `file:line` evidence, target architecture, and a phased plan (MSCM-P0–P6). | To understand what multi-provider support actually costs, and what must land *before* the plugin is published publicly. |
...
| **[../scripts/bruno-catalog/](../scripts/bruno-catalog)** | The Bruno-collection → catalog generator: five production collections from `github.com/bruno-collections` emitted as `kind: API` entities with an **OpenAPI 3.0.3 definition generated from each collection**. Documents the mapping rules and the fact that `@usebruno/converters` has no `brunoToOpenApi` direction. | To understand where `examples/bruno-collections.yaml` comes from, or to add/change a source collection. |
