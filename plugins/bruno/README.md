...
| --- | --- | --- |
| `descriptor` | a `catalog-info.yaml` in source control — hand-authored, or generated by this flow's **Create pull request** | edit that file — by pull request when the host is GitHub or GitLab, and by copying the computed YAML otherwise |
| `ui` | registered with the backend by this dashboard's **Add collection**, and stored only there | no descriptor file to open a pull request against, so `spec.partOf` is changed by linking in this instance instead (with `bruno.allowRuntimeWrites` on). To move it into source control, commit and register a `catalog-info.yaml` and then Remove the dashboard's copy — the Remove action is offered on this origin only |
...
Pull requests are opened against **GitHub and GitLab**, through the adapters in
[`src/lib/pr/`](src/lib/pr/), and the host app's configured `scmIntegrationsApi`
decides which URLs are which — including self-hosted ones. That is why capability
is resolved from the integrations registry rather than from this plugin's own
`scmProviderFromUrl`, which guesses from the hostname and is only meant for
validating pasted input. On every other host the dialogs show the finished
`spec.partOf` YAML with a copy button and the repo-relative descriptor path, so
nothing dead-ends; see [Pull requests, per host](#pull-requests-per-host).
...
**Add collection** appears only where `bruno.allowRuntimeWrites` is on. With it
off `AddCollectionAction` withholds the `onAdd` callback, and the dialog — whose
only knowledge of the flag is that absence — drops the button and stops framing
the ending as a choice. The backend refuses the create either way; hiding it is
...
not in an SCM, and a host with no `lib/pr/` adapter cannot be committed to from
here. Those five cases used to dead-end the dialog with a paragraph about what to
edit by hand. It still explains, as the reason the pull-request radio is
disabled rather than as the end of the flow — and for the last of the five it
now shows the finished `spec.partOf` YAML with a copy button and the
repo-relative path, since working that edit out needs no token and no network.

The runtime option itself is available only where `bruno.allowRuntimeWrites` is
on. `useLinkMethod` takes both possibilities and collapses to whichever is
real, so the dialog has three shapes rather than one: the radio pair when both
are open, a single paragraph when only the pull request is (a group with one
selectable option asks for a decision already made), and — for those five cases
on an instance with runtime writes off — the advice alone and a Close, which is
the dead end restored, honestly this time.
...
[`src/lib/unlinkPr.ts`](src/lib/unlinkPr.ts), over the `PrAdapter` seam in
[`src/lib/pr/`](src/lib/pr/). The token comes from
...
### Pull requests, per host — [`src/lib/pr/`](src/lib/pr/)

`unlinkPr.ts` and `descriptorPr.ts` hold the YAML edit, the plan/submit split and
the wording. Everything forge-specific is behind `PrAdapter`
([`types.ts`](src/lib/pr/types.ts)): parse a descriptor URL, build the repo URL,
read the file, resolve the default branch, open the request.
[`registry.ts`](src/lib/pr/registry.ts) dispatches on
`scmIntegrationsApi.byUrl(url)?.type`, the same key
`plugins/bruno-backend/src/scm/index.ts` reads.

| Host | What happens |
| --- | --- |
| **GitHub** ([`github.ts`](src/lib/pr/github.ts)) | contents/git/pulls through Octokit, with the integration's `apiBaseUrl` threaded so GitHub Enterprise works |
| **GitLab** ([`gitlab.ts`](src/lib/pr/gitlab.ts)) | files/commits/merge_requests over `fetch`, project addressed by URL-encoded `path_with_namespace` |
| **Everything else** | the computed `spec.partOf` YAML, with a copy button and the exact repo-relative path |

The one abstraction the seam adds is **`concurrencyToken`** — GitHub's blob `sha`,
GitLab's `last_commit_id`, Bitbucket's parent sha. All three mean "fail if this
moved under me", all three are opaque strings, and `undefined` means *create,
do not update*, which is what makes `descriptorPr.ts` refuse to overwrite a
descriptor someone else wrote.

GitLab needs **no app-side change**. `ScmAuth.forGitlab`'s stock `repoWrite`
mapping is `['write_repository', 'api']`
(`node_modules/@backstage/integration-react/dist/api/ScmAuth.esm.js`), and `api`
is the scope the REST endpoints want — `write_repository` alone covers only
git-over-HTTP. GitLab also needs no separate create-branch call: a commit with
`start_branch` creates the head branch as part of the commit, so a rejected
commit leaves no orphan branch behind.

**Bitbucket Cloud is deliberately absent**, and not for want of an API — the Cloud
endpoints exist. It needs a scope this app does not ask for:
`packages/app/src/modules/auth/scmAuth.ts` maps Bitbucket's `repoWrite` to `[]`,
and even `ScmAuth`'s stock Bitbucket mapping omits `repository:write`. Widening
what every Bitbucket user consents to is an app-side decision, so a Bitbucket
descriptor takes the copy-the-YAML path until it is made.

**Why these writes stay in the browser.** Backstage's scaffolder modules open
pull requests against all three forges, and delegating to them is *eliminated*,
not merely unchosen: they are backend actions, this flow runs on the user's own
`scmAuth` token, and the whole point is that the pull request is authored by the
user. Server-side has exactly two forms — every PR authored by a service account,
or the user's token forwarded to our backend — and
[`plugins/bruno-backend/README.md`](../bruno-backend/README.md) refuses the
second outright.
